...
class setting_config:
    ...
    """
    # 获取当前文件（main.py）所在目录的路径
    current_dir = os.path.dirname(os.path.abspath(__file__))
    # 获取 data2 文件夹的路径
    data_dir = os.path.join(os.path.dirname(current_dir), 'pre_trained_weights')
    network = 'vmunet'
    model_config = {
        'num_classes': 1,
        'input_channels': 3,
        ...
        'drop_path_rate': 0.2,
        'load_ckpt_path': data_dir + '/vmamba_small_e238_ema.pth',
        #'load_ckpt_path': '/root/autodl-tmp/VM-UNet/results_point/vmunet_isic17_Wednesday_17_July_2024_20h_14m_07s/checkpoints/best.pth',
        'resume_ckpt': None,
    }
    # 获取当前文件（main.py）所在目录的路径
    current_dir = os.path.dirname(os.path.abspath(__file__))
    ...
    if datasets == 'isic18':
        data_path = data_dir+'/isic2018/'
    elif datasets == 'isic17':
        data_path = data_dir + common_data
    else:
    ...
    work_dir = os.path.dirname(current_dir) + '/results/' + network + '_' + datasets + '_' + datetime.now().strftime(
        '%A_%d_%B_%Y_%Hh_%Mm_%Ss') + '/'
    os.makedirs(work_dir, exist_ok=True)
    print_interval = 20
    val_interval = 30
    # 要想输入一张保存一张，把save_interval改成1
    save_interval = 100
    threshold = 0.5
...
class setting_config_point:
    ...
    model_config = {
        ...
        'depths_decoder': [2,2,2,1],
        'drop_path_rate': 0.2,
        'load_pretrained': data_dir+'/best-epoch142-loss0.3488.pth',
        'load_ckpt_path': data_dir+'/vmamba_small_e238_ema.pth',
        #'load_ckpt_path':'/root/autodl-tmp/VM-UNet/results_point/vmunet_isic17_Wednesday_17_July_2024_20h_14m_07s/checkpoints/best.pth',
    }
    # 获取当前文件（main.py）所在目录的路径
    ...
    work_dir = os.path.dirname(current_dir) + '/results_point/' + network + '_' + datasets + '_' + datetime.now().strftime(
        '%A_%d_%B_%Y_%Hh_%Mm_%Ss') + '/'
    os.makedirs(work_dir, exist_ok=True)
    print_interval = 20
    val_interval = 30
...
class setting_config_predict:
    ...
    model_config = {
        ...
        'drop_path_rate': 0.2,
        'load_ckpt_path': data_dir + '/vmamba_small_e238_ema.pth',
        #'load_ckpt_path': "/root/results/vmunet_isic17_Wednesday_17_July_2024_17h_41m_26s/checkpoints/best.pth",
    }
    # 获取当前文件（main.py）所在目录的路径
    current_dir = os.path.dirname(os.path.abspath(__file__))
    datasets = 'isic17'
